# LanDataResponseHandleModel.py —— 原 Java 参考实现

由 `custom_components/leelen_home/leelen/models/LanDataResponseHandleModel.py` 中注释掉的 Java 源码整理而来(纯参考,不参与运行)。
保留它是为了对照协议语义;Python 实现以源码为准。

## handle_get_device_status()  (源码原第 399-842 行)
...
## handle_config_modify_notify()  (源码原第 887-1899 行)
